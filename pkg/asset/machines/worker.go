...
func (w *Worker) Generate(dependencies asset.Parents) error {
	...
	for _, pool := range ic.Compute {
		...
		case alibabacloudtypes.Name:
			...
			sets, err := alibabacloud.MachineSets(clusterID.InfraID, ic, &pool, "worker", "worker-user-data-managed", installConfig.Config.Platform.AlibabaCloud.Tags)
			if err != nil {
		...
		case ibmcloudtypes.Name:
			...
			sets, err := ibmcloud.MachineSets(clusterID.InfraID, ic, &pool, "worker", "worker-user-data-managed")
			if err != nil {
				return errors.Wrap(err, "failed to create worker machine objects")
			}
			for _, set := range sets {
		...
		case openstacktypes.Name:
			...
			sets, err := openstack.MachineSets(clusterID.InfraID, ic, &pool, imageName, "worker", "worker-user-data-managed", nil)
			if err != nil {
				return errors.Wrap(err, "failed to create worker machine objects")
			}
			for _, set := range sets {
