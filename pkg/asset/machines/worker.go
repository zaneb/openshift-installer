...
func (w *Worker) Generate(dependencies asset.Parents) error {
	...
	for _, pool := range ic.Compute {
		...
		case alibabacloudtypes.Name:
			...
			sets, err := alibabacloud.MachineSets(clusterID.InfraID, ic, &pool, "worker", workerUserDataSecretName, installConfig.Config.Platform.AlibabaCloud.Tags)
			if err != nil {
		...
		case baremetaltypes.Name:
			...
			sets, err := baremetal.MachineSets(clusterID.InfraID, ic, &pool, string(*rhcosImage), "worker", workerUserDataSecretName)
			if err != nil {
				return errors.Wrap(err, "failed to create worker machine objects")
			}
			for _, set := range sets {
